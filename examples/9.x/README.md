...
```bash
# Should use version 9.1.2 for the default version
lando exec defaults -- curl -s -XGET localhost:9200 | grep "number" | grep 9.1.2
...
# Should use version 9.0.3 for the patch service
lando exec patch -- curl -s -XGET localhost:9200 | grep "number" | grep 9.0.3
```
